...
const scssEntries = scssFiles.reduce((entries, filePath) => {
    // The shape gallery has two SCSS files: index.scss (the partial that consumers @use to opt
    // into individual shapes) and master.scss (the dist-bundle entry that installs all shapes).
    // Only the latter should be compiled as a webpack entry.
    ...
    // Foundations TS are built via tsEntries.
